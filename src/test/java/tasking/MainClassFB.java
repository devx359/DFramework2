...
import lib.FireBaseUtil;
import lib.Rethink_query;
import lib.TaskAdd;

public class MainClassFB {
	...
	public void defaultSteps(Hashtable<String, String> tdata) throws ParseException {
		...
		try {
			...
			System.out
					.println("Excel file read issue: " + e2 + " in sheet: " + sheetname + " " + CurrentTestCaseSerial);
	...
	public void task_send_to_operator() {
		System.out.println("FireBase API Setter SDK Called for: " + userCode + " node: " + opNode);
		//***************SDK Call ************
		boolean resp = fbUtil.initiateFirestore(node_tsk_id, userCode, port);

		if (resp == true) {
			taskfound = true;
		} else {
			test.fail("FireBase API Setter SDK Call Failed" + " for: " + userCode + " node: " + opNode);
			taskfound = false;
		}
		...
		while (count <= nooftasks) {
			...
			if (job_name.equalsIgnoreCase("multi_psubmit_flow")) { // For multi P_submit jobs
				while (p_submit_count <= batch_size) {
					...
					System.out.println("Task SUBMIT API Request for: " + userCode + " node: " + opNode );
			...
			/*	try {
					res = given().header("Authorization", token) .filter(new RequestLoggingFilter()) 
							.contentType(ContentType.JSON).body(jsonreq).when().contentType(ContentType.JSON).post()
							.then().contentType(ContentType.JSON).extract().response();
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
	...
	public void requeue() throws InterruptedException {
		...
		if (time >= 300000) {
			System.out
					.print("Task not processed after waiting..Aborting Requeue " + nodesTMasterid + " " + nodetaskcode);
	...
	public void rework() throws InterruptedException {
		...
		if (IsTaskMysqlProccessedState == true) { // if it is set to true then wait for the task to be processed in
			...
			if (time >= 300000) {
				System.out.println("Task not processed in mysql engagement table ..Aborting Rework API call "
						+ nodesTMasterid + " " + nodetaskcode);
				test.fail("Task not processed in mysql engagement table..Aborting Rework API call " + nodesTMasterid
						+ " " + nodetaskcode);
				AbortRework = true;
			}
		}
