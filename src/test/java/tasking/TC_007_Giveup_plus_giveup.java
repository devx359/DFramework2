...
public class TC_007_Giveup_plus_giveup {
	...
	public void setup() {
		...
		reports = ExtentManagerObj.GetExtent("TC_007_GiveUp"+nodeId);
		test = reports.createTest("GiveUp+GiveUp "+nodeId);
	...
	public void task_request() {
		...
		if (status.equalsIgnoreCase("dispatched")) {
			...
			test.warning("JSON Response " + res.asString() + " Status code " + res.statusLine());
		}
	...
	public void task_submit() {
		...
		while (count <= nooftasks) {
			...
			test.info("************task_GiveUp API Request***************** ");
			test.info("API Request " + jsonreq);
			...
			// task response
			test.info("Table task_response data " + rethink.task_response_with_status_change_processed_GiveUp().toString());
			...
			if (count == 1) {
				task_code_givenup = JsonPath.read(tsklist1, "$.task_code");
			} else {
				task_code_new = JsonPath.read(tsklist1, "$.task_code");
			}
			...
			if (task_code_givenup.equalsIgnoreCase(task_code_new)) {
				...
				test.pass("After request same task was not assigned=>New TASK CODE: " + task_code_new+" 1st TASK CODE:"+task_code_givenup);
			}
