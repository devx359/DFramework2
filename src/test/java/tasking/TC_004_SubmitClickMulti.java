...
//Multiple Submits with same data

public class TC_004_SubmitClickMulti {
	...
	public void setup() {
		...
		test = reports.createTest("Serial tasks Scenario Multi Submit-For Node: " + nodeId);
	...
	public void task_request() {
		...
		try {
			...
			e1.printStackTrace();
			test.fatal("API Coonection Issue: "+e1);
		}
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			test.info("Task Request query data not found");
		}
	...
	public void task_submit() throws InterruptedException {
		...
		while (count <= nooftasks) {
			...
			test.info("*******************TASK SUBMIT************************   Task#" + count + " node_tsk_id: " + node_tsk_id);
			System.out.println("*********Task submit*********   Task#" + count + " node_tsk_id: " + node_tsk_id);
		...
		/*	if (count > 1) {
				 ...
				 * 
				 * Map map = (Map) ob; node_tsk_id = map.get("node_task_id").toString();
				 * 
				 * reques_id = next_reques_id;
				 
				test.info("-------Table details after Same task request-----");
				// Fetch tables
			...
			test.info("**********task_submit API Request************ for node_task_id: "+node_tsk_id+" |userCode: "+user_code+" |port:"+port);
			test.info("API Request " + jsonreq);
			...
			+" |node_task_id"+JsonPath.read(noderes1, "$.node_task_id")
			+" |task_code"+JsonPath.read(noderes1, "$.task_code"));
